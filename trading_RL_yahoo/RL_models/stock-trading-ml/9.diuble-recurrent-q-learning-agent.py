    ...
    def buy(self, initial_money):
        ...
        for t in range(0, len(self.trend) - 1, self.skip):
            ...
            if action == 1 and initial_money >= self.trend[t]:
                ...
                if not os.path.isfile('9.csv'):
                    df1.to_csv('9.csv', index=False)
                else:
                    df1.to_csv('9.csv', index=False, mode='a', header=False)
                ...
                if not os.path.isfile('9.csv'):
                    df2.to_csv('9.csv', index=False)
                else:
                    df2.to_csv('9.csv', index=False, mode='a', header=False)
            else:
                print(
                ...
                if not os.path.isfile('9.csv'):
                    df3.to_csv('9.csv', index=False)
                else:
                    df3.to_csv('9.csv', index=False, mode='a', header=False)

            new_state = np.append([self.get_state(t + 1)],
                                  self.INITIAL_FEATURES[:3, :], axis=0)
            self.INITIAL_FEATURES = new_state
        fi = pd.read_csv('9.csv')
        print(fi.tail(2))

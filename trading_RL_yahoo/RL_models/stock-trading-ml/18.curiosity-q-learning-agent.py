    ...
    def buy(self, initial_money):
        ...
        for t in range(0, len(self.trend) - 1, self.skip):
            ...
            if action == 1 and initial_money >= self.trend[t]:
                ...
                if not os.path.isfile('18.csv'):
                    df1.to_csv('18.csv', index=False)
                else:
                    df1.to_csv('18.csv', index=False, mode='a', header=False)            
                ...
                if not os.path.isfile('18.csv'):
                    df2.to_csv('18.csv', index=False)
                else:
                    df2.to_csv('18.csv', index=False, mode='a', header=False)                
            else:
                ...
                if not os.path.isfile('18.csv'):
                    df3.to_csv('18.csv', index=False)
                else:
                    df3.to_csv('18.csv', index=False, mode='a', header=False)

            state = next_state

        fi = pd.read_csv('18.csv')
        print(fi.tail(2))

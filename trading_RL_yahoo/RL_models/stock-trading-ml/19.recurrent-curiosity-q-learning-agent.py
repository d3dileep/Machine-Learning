...
class Agent:
    ...
    def __init__(self, state_size, window_size, trend, skip):
        ...
        with tf.variable_scope('curiosity_model'):
            ...
            cell = tf.nn.rnn_cell.LSTMCell(
                self.LAYER_SIZE, state_is_tuple=False)
            self.rnn, last_state = tf.nn.dynamic_rnn(inputs=state_action, cell=cell,
                                                     dtype=tf.float32,
                                                     initial_state=self.hidden_layer)
            self.curiosity_logits = tf.layers.dense(
                self.rnn[:, -1], self.state_size)
        ...
        t_params = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope='q_model/target_net')
        e_params = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope='q_model/eval_net')
        self.target_replace_op = [tf.assign(t, e)
    ...
    def _memorize(self, state, action, reward, new_state, done, rnn_state):
        self.MEMORIES.append(
            (state, action, reward, new_state, done, rnn_state))
    ...
    def buy(self, initial_money):
        ...
        for t in range(0, len(self.trend) - 1, self.skip):
            ...
            if action == 1 and initial_money >= self.trend[t]:
                ...
                if not os.path.isfile('19.csv'):
                    df1.to_csv('19.csv', index=False)
                else:
                    df1.to_csv('19.csv', index=False, mode='a', header=False)
                ...
                if not os.path.isfile('19.csv'):
                    df2.to_csv('19.csv', index=False)
                else:
                    df2.to_csv('19.csv', index=False, mode='a', header=False)
            else:
                ...
                if not os.path.isfile('19.csv'):
                    df3.to_csv('19.csv', index=False)
                else:
                    df3.to_csv('19.csv', index=False, mode='a', header=False)

            new_state = np.append([self.get_state(t + 1)],
                                  self.INITIAL_FEATURES[:3, :], axis=0)
            self.INITIAL_FEATURES = new_state
        fi = pd.read_csv('19.csv')
        print(fi.tail(2))

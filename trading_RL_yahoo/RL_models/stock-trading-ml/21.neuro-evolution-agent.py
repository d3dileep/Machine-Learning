...
class neuralnetwork:
    def __init__(self, id_, hidden_size=128):
        self.W1 = np.random.randn(
            window_size, hidden_size) / np.sqrt(window_size)
...
class NeuroEvolution:
    ...
    def mutate(self, individual, scale=1.0):
        mutation_mask = np.random.binomial(
            1, p=self.mutation_rate, size=individual.W1.shape)
        individual.W1 += np.random.normal(loc=0, scale=scale,
                                          size=individual.W1.shape) * mutation_mask
        mutation_mask = np.random.binomial(
            1, p=self.mutation_rate, size=individual.W2.shape)
        individual.W2 += np.random.normal(loc=0, scale=scale,
                                          size=individual.W2.shape) * mutation_mask
        return individual
    ...
    def buy(self, individual):
        ...
        for t in range(0, len(self.trend) - 1, self.skip):
            ...
            if action == 1 and starting_money >= self.trend[t]:
                ...
                if not os.path.isfile('21.csv'):
                    df1.to_csv('21.csv', index=False)
                else:
                    df1.to_csv('21.csv', index=False, mode='a', header=False)
                ...
                try:
                    invest = (
                        (self.trend[t] - bought_price) / bought_price) * 100
                except:
                    invest = 0
                print(
                ...
                if not os.path.isfile('21.csv'):
                    df2.to_csv('21.csv', index=False)
                else:
                    df2.to_csv('21.csv', index=False, mode='a', header=False)
            else:
                print('day %d, hold UNIT at price %f,  total balance %f,' %
                      (t+1, close[t+1], initial_money))
                df3 = pd.DataFrame(
                    {'Date': date1[t+1], 'Close': [close[t+1]], 'RESULT': ['Hold']})
                if not os.path.isfile('21.csv'):
                    df3.to_csv('21.csv', index=False)
                else:
                    df3.to_csv('21.csv', index=False, mode='a', header=False)

            state = next_state
        fi = pd.read_csv('21.csv')
        print(fi.tail(2))
    ...
    def evolve(self, generations=15, checkpoint=5):
        ...
        for epoch in range(generations):
            ...
            total_fitness = np.sum([np.abs(i.fitness)
                                    for i in self.population])
            parent_probabilities = [
                np.abs(i.fitness / total_fitness) for i in self.population]

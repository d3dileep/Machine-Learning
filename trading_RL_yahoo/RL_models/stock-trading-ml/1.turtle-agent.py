...
def buy_stock(real_movement,signal,initial_money = 10000,max_buy = 1,max_sell = 1,):
	...
	def buy(i, initial_money, current_inventory):
		...
		if shares < 1:
			...
			df1 = pd.DataFrame({'Date': date1[i], 'Close': [close[i]], 'RESULT': ['Buy']})
			if not os.path.isfile('1.csv'):
				df1.to_csv('1.csv', index=False)
			else:
				df1.to_csv('1.csv', index=False, mode='a', header=False)
			states_buy.append(0)
	...
	for i in range(real_movement.shape[0] - int(0.025 * len(df))):
		...
		if state == 1:
			...
			if current_inventory == 0:
				...
				df2 = pd.DataFrame({'Date': date1[i], 'Close': [close[i]], 'RESULT': ['Sell']})
				if not os.path.isfile('1.csv'):
					df2.to_csv('1.csv', index=False)
				else:
					df2.to_csv('1.csv', index=False, mode='a', header=False)
			

			states_sell.append(i)
	fi = pd.read_csv('1.csv')
	print(fi.tail(2))

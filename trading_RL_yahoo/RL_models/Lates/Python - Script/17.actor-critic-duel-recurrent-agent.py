        ...
        for t in range(0, len(self.trend) - 1, self.skip):
            ...
            if action == 1 and initial_money >= close[t]:
                ...
                if not os.path.isfile('output/17.actor-critic-duel-recurrent-agent.csv'):
                    df1.to_csv('output/17.actor-critic-duel-recurrent-agent.csv', index=False)
                else:
                    df1.to_csv('output/17.actor-critic-duel-recurrent-agent.csv', index=False, mode='a', header=False)
...
fi = pd.read_csv("output/17.actor-critic-duel-recurrent-agent.csv")
print(fi.tail(2))